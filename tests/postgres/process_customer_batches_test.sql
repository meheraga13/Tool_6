-- PostgreSQL Test Script for process_customer_batches
DO $$ 
DECLARE 
...
BEGIN
    ...
    BEGIN
        -- Call the procedure
        CALL process_customer_batches(1001, result_msg_1);
        RAISE NOTICE 'Test %: % - Completed', test_count, 'Valid input test';
        RAISE NOTICE 'Output parameter result_msg: %', result_msg_1;

        
        -- Perform verifications
        SELECT COUNT(*) INTO verification_count FROM #temp_jobs WHERE status = 'active';
    ...
    BEGIN
        -- Call the procedure
        CALL process_customer_batches(NULL, result_msg_1);
        RAISE NOTICE 'Test %: % - Completed', test_count, 'Boundary value test';
        RAISE NOTICE 'Output parameter result_msg: %', result_msg_1;

        
        -- Perform verifications
        SELECT COUNT(*) INTO verification_count FROM #temp_jobs WHERE status IS NULL OR status = 'pending';
        IF verification_count > 0 THEN 
    ...
    BEGIN
        -- Call the procedure
        CALL process_customer_batches(-1, result_msg_1);
        RAISE NOTICE 'Test %: % - Completed', test_count, 'Error condition test';
        RAISE NOTICE 'Output parameter result_msg: %', result_msg_1;

        
        -- Perform verifications
        SELECT COUNT(*) INTO verification_count FROM #temp_jobs WHERE error_flag = true;
        IF verification_count > 0 THEN 
