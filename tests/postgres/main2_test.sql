-- PostgreSQL Test Script for main2
DO $$ 
DECLARE 
...
BEGIN
    ...
    BEGIN
        -- Call the procedure
        CALL main2(NULL, result_msg_1);
        RAISE NOTICE 'Test %: % - Completed', test_count, 'Boundary value test';
        RAISE NOTICE 'Output parameter result_msg: %', result_msg_1;

        
        -- Perform verifications
        SELECT COUNT(*) INTO verification_count FROM #my_temp_table WHERE status IS NULL OR status = 'pending';
        IF verification_count > 0 THEN 
    ...
    BEGIN
        -- Call the procedure
        CALL main2(-1, result_msg_1);
        RAISE NOTICE 'Test %: % - Completed', test_count, 'Error condition test';
        RAISE NOTICE 'Output parameter result_msg: %', result_msg_1;

        
        -- Perform verifications
        SELECT COUNT(*) INTO verification_count FROM #my_temp_table WHERE error_flag = true;
        IF verification_count > 0 THEN 
